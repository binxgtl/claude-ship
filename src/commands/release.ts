...
import { getAllFilePaths, filterFilesForGit } from "../scaffold.js";
...
export async function runRelease(opts: ReleaseOptions) {
  ...
  if (apiKey) {
    ...
    try {
      changelogContent = await generateChangelog({
        ...
        apiKey,
        count: 50,
        onChunk: (chunk) => { spinChangelog.stop(); process.stdout.write(chunk); },
      });
      console.log();
  ...
  await initAndCommit(dir, gitFiles, `release: ${tag}`);
